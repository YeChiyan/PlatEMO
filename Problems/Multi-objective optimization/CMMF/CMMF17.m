classdef CMMF17 < PROBLEM
    ...
    methods
        ...
        function PopObj = CalObj(obj, X)
            ...
            % Local Angle Calculation for 4 quadrants
            THETA = zeros(N, 1);
            for i = 1 : N
                if X(i, 1) >= 0 && X(i, 2) >= 0
            ...
            for i = 1 : N
                ...
                elseif X(i, 1) < 0 && X(i, 2) >= 0
                    % Peak 2: Circle r=1 at (-1,1)
                    T(i) = (1 - ((X(i, 1)+1)^2 + (X(i, 2)-1)^2))^2 + h(i);
                elseif X(i, 1) < 0 && X(i, 2) < 0
                    % Peak 3: Circle r=1 at (-1,-1)
                    T(i) = (1 - ((X(i, 1)+1)^2 + (X(i, 2)+1)^2))^2 + h(i);
                else
                    % Peak 4: Circle r=1 at (1,-1)
                    T(i) = (1 - ((X(i, 1)-1)^2 + (X(i, 2)+1)^2))^2 + h(i);
                end
            end
        ...
        function PopCon = CalCon(obj, X)
            ...
            PopCon = zeros(N, 2);
            for i = 1 : N
                % 1. Distance band constraints for 4 peaks
                if X(i, 1) >= 0 && X(i, 2) >= 0
                    val = X(i, 1)^2 + X(i, 2)^2;
                elseif X(i, 1) < 0 && X(i, 2) >= 0
                    val = (X(i, 1)+1)^2 + (X(i, 2)-1)^2;
                elseif X(i, 1) < 0 && X(i, 2) < 0
                    val = (X(i, 1)+1)^2 + (X(i, 2)+1)^2;
                else
                    val = (X(i, 1)-1)^2 + (X(i, 2)+1)^2;
                end
                % Feasible band: [0.9, 1.1]
        ...
        function R = GetOptimum(obj, N)
            ...
            C2 = [-1 + r*cos(phi), 1 + r*sin(phi)];
            C3 = [-1 + r*cos(phi), -1 + r*sin(phi)];
            C4 = [1 + r*cos(phi), -1 + r*sin(phi)];
