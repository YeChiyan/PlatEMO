...
% Fix: Use DE Operator for tighter convergence (压缩云团)
...
for i = 1:length(HD)
    ...
    if n_sub >= 4
        % [核心修改] 对于正常簇，使用 DE 算子
        % DE 擅长在平坦区域利用种群分布信息进行快速收敛
        Off = OperatorDE(Problem, subpop, {1, 0.5, 1, 20});
        lpop{i} = [lpop{i} Off];
        
    elseif n_sub >= 2
        % 如果种群太小(2-3个)，DE跑不起来，回退到 GA
        mateidx = TournamentSelection(2, n_sub, -lCD{i});
