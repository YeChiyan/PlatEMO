...
%         Next(Temp(Del)) = false;
        ...
        Index=find(Next==true);
            ...
            dobj = sort(pdist2(Population2.objs,Population2.objs)); dobj = sum(dobj(1:K,:));%dobj./max(max(dobj));
            ddec = sort(pdist2(Population2.decs,Population2.decs)); ddec = sum(ddec(1:K,:));%ddec./max(max(ddec));
